...
function lapWrapSDDM(sddmSolver, a::AbstractArray; tol::Real=1e-6, maxits=Inf, maxtime=Inf, verbose=false, pcgIts=Int[], params...)
    ...
    f = Graphoskop.lapWrapComponents(Graphoskop.lapWrapConnected(sddmSolver))(a, tol=tol, maxits=maxits, maxtime=maxtime, verbose=verbose, pcgIts=pcgIts)
...
function lapWrapSDDM(sddmSolver)
    ...
    return Graphoskop.lapWrapComponents(Graphoskop.lapWrapConnected(sddmSolver))
